...
% load 'C:\Users\MohammadArafat\Dropbox\CS US\cyst_d40x40_c5s40_nrf.mat'
% D1 = 256.*(rf./max(rf(:)));
% D1 = downsample(D1,2);

% load 'C:\Users\MohammadArafat\Dropbox\Elastography\AM2D\rf052.mat'
% rf = RfDataFilt(1:2048,233:360);clear RfDataFilt;
% rf = downsample(rf,2);
% D1 = 256.*(rf./max(rf(:)));

% load 'C:\Users\MohammadArafat\Dropbox\Elastography\AM2D\rf021.mat'
% rf = RfDataFilt(1:2048,76:203);clear RfDataFilt;
% rf = downsample(rf,2);
% D1 = 256.*(rf./max(rf(:)));

% load 'C:\Users\MohammadArafat\Dropbox\Elastography\AM2D\rf011.mat'
% rf = RfDataFilt(1:2304,251:378);clear RfDataFilt;
% rf = downsample(rf,2);
% D1 = 256.*(rf./max(rf(:)));

% load 'C:\Users\MohammadArafat\Dropbox\Elastography\AM2D\rf043.mat'
...
for bb = 1:2
    ...
    if bb == 1   
        load 'C:\Users\arafat\Dropbox\Elastography\distal_radius_pre.mat'
        rf = distal_radius_pre(1:2048,1:128);clear distal_radius_pre;
        rf = downsample(rf,2);
        D1 = 256.*(rf./max(rf(:)));
    else
        load 'C:\Users\arafat\Dropbox\Elastography\distal_radius_pst.mat'
        rf = distal_radius_pst(1:2048,1:128);clear distal_radius_pst;
        rf = downsample(rf,2);
        D1 = 256.*(rf./max(rf(:)));
    end
...
%             MSSIM(nn,mm) = ssim3;
    ...
    save(['curvelet2_' num2str(bb) '.mat'],'recon_c');
    save(['waveatom2_' num2str(bb) '.mat'],'recon_w');
    save(['combined2_' num2str(bb) '.mat'],'recon');
end
